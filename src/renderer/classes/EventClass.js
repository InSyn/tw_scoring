...
import { checkCompetitionDiscipline, getDefaultStages, isQualificationOfDisciplines } from '../data/sports';
...
export default class EventClass {
        ...
        formulas: [
          ...
          },
          {
            ...
            get_result: (comp_id, race_id, judges, runParameters) => {
              ...
              const runTime = !isNaN(runParameters.runTime) ? Number(runParameters.runTime) : 0;
              ...
              const turnMarks = marks.filter((mark) => {
                return (
                  mark.race_id === race_id &&
                  !(mark.moguls_value.baseScore === null || mark.moguls_value.baseScore === undefined) &&
                  !(mark.moguls_value.deduction === null || mark.moguls_value.deduction === undefined)
                );
              });
              const baseScores = cutMarks(
                turnMarks.map((judgeScore) => {
                  return Number(judgeScore.moguls_value.baseScore);
                }),
                maxMarksCount,
                minMarksCount
              );
              const deductionScores = cutMarks(
                turnMarks.map((judgeScore) => {
                  return Number(judgeScore.moguls_value.deduction);
                }),
                maxMarksCount,
                minMarksCount
              );
              const _turnsSum =
                baseScores.reduce((base1, base2) => Number(base1) + Number(base2), 0) -
                deductionScores.reduce((deduction1, deduction2) => Number(deduction1) + Number(deduction2), 0);

              const minTurnsSum = !isNaN(turnMarks.length) ? roundNumber((Number(turnMarks.length) - maxMarksCount - minMarksCount) * 0.1, 1) : 0;
              const turnsSum = _turnsSum < minTurnsSum ? minTurnsSum : _turnsSum;
              ...
              const jump1_scores = marks
                .map((mark) => (mark.moguls_value.jump1_score !== undefined ? truncateNumber(mark.moguls_value.jump1_score * jump1_coef, 2) : 0))
                .filter((mark) => !!mark);
              const jump2_scores = marks
                .map((mark) => (mark.moguls_value.jump2_score !== undefined ? truncateNumber(mark.moguls_value.jump2_score * jump2_coef, 2) : 0))
                .filter((mark) => !!mark);
                ...
                _section.judges.forEach((_judge) => {
                  section.push(
                    (this.competitorsSheet.competitors
                      ...
                      .marks.find(
                        (_mark) => _mark.race_id === race_id && _mark.judge.toString() === _judge.id.toString() && _mark.section === _section.s_num
                      ) &&
                      this.competitorsSheet.competitors
                        .find((_comp) => {
                          return _comp.id === comp_id;
                        ...
                        .marks.find((_mark) => {
                          return _mark.race_id === race_id && _mark.judge.toString() === _judge.id.toString() && _mark.section === _section.s_num;
                        }).value) ||
                      0
                  );
                });
  ...
  getRaceResult(competitor, race) {
    ...
    return result
      ? result.status
        ? result.status
        : `${result.value_str ? result.value_str : result.value || this.roundWithPrecision(0)}${result.repeat ? ' ' + result.repeat : ''}`
      : this.roundWithPrecision(0);
  }
  ...
  publishResult(params) {
    ...
    const resultValue = manualResult
      ? manualResult
    ...
    const res = {
      id: generateId(),
      value: resultValue,
      value_str: isTimingResult ? formatTimeDifference(resultValue, { precision: this.structure.selected.accuracy, format: 'short' }) : null,

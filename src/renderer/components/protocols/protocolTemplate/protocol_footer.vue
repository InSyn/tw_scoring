  ...
  >
    ...
    >
      <div style="padding: 2px 4px; margin-right: auto">
        {{
          `${getCompetitionDate()} / ${competition.mainData.location.value}(${
      ...
      <div style="margin-left: auto">
        {{
          `Отчёт создан ${date_now[0]} ${date_now[1]} / Страница ${
            page_index + 1
          }/${(paginated_results.length > 0 && paginated_results.length) || 1}`
        }}
      </div>
    </div>
    <div
    ...
    <div
      style="width: 100%; display: flex; align-items: center; flex-shrink: 0"
    >
      <div
        style="
          margin-right: auto;
  ...
  methods: {
    getCompetitionDate() {
      ...
      const competitionDate = new Date(
        Date.parse(this.competition.mainData.date.value)
      );

      return competitionDate.toLocaleDateString("ru", {
        year: "numeric",
        month: "long",
        day: "numeric",
      });

<template>
  <div class="wrapper flex-column align-center" style="min-height: 100%">
    <div class="pa-2" style="min-height: 100%; border-radius: 4px; background-color: var(--background-card)">
      <div style="display: flex; align-items: center; flex-wrap: nowrap; font-weight: bold; font-size: 1.2rem">
        <v-dialog v-model="competition.technicalInfo.change_dialog" overlay-opacity="100%" width="320px"
          ><template v-slot:activator="{ on }"
            ><v-btn v-on="on" small icon :color="$vuetify.theme.themes[appTheme].accent_light"><v-icon small>mdi-tools</v-icon></v-btn></template
          ><<v-card class="section-container"
            ...
            <v-card-actions class="d-flex justify-end pa-1"
              ><v-btn small @click.stop="competition.technicalInfo.change_dialog = false" :color="$vuetify.theme.themes[appTheme].textDefault"
                >Close</v-btn
              ></v-card-actions
            ></v-card
          ></v-dialog
      ...
      >
        ...
        <v-col class="d-flex justify-center align-center" cols="1"
          ><v-btn @click="competition.technicalInfo.records.splice(competition.technicalInfo.records.indexOf(techString), 1)" icon small
            ><v-icon small :color="$vuetify.theme.themes[appTheme].action_red">mdi-close</v-icon></v-btn
          ></v-col
        >
      </v-row>
    </div>

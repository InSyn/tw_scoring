<template>
  <div class="flex-column pa-2" style="flex: 0 0 auto; border-radius: 4px; background-color: var(--background-card)">
    <div class="d-flex align-center">
      ...
      <v-btn text style="margin-left: auto" :color="$vuetify.theme.themes[appTheme].accent" @click="addWeather()">
    ...
    <v-row v-for="(w_row, i) in competition.weather" :key="i" style="font-size: 0.9rem" no-gutters>
      <v-col class="d-flex align-center py-1" cols="4">
        <input
          ...
          v-model="competition.weather[i].descr1"
      /></v-col>
      ...
      <v-col cols="1" style="display: flex; align-items: center">
        <v-btn small icon @click="removeWeather(i)" :color="$vuetify.theme.themes[appTheme].action_red">

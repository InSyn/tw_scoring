    ...
    <div class="pa-2" style="display: flex; flex-direction: column; height: 100%; width: 100%; background-color: var(--background-card); border-radius: 4px">
      <div style="flex: 0 0 auto">
        <div class="d-flex flex-wrap align-center" style="padding: 2px 4px">
          ...
          />
          <label class="d-inline-block font-weight-bold" for="port" style="margin-left: 1rem">Port</label>
          <input
            id="port"
        ...
        <div class="mt-1 mb-2 d-flex align-center">
          <v-btn :color="$vuetify.theme.themes[appTheme].success" :disabled="serverStatus" style="padding: 2px 4px" text @click="startServer()">
            {{ localization[lang].app.server.start_btn }}
            <v-icon :color="$vuetify.theme.themes[appTheme].success"> mdi-play </v-icon>
          </v-btn>
          <div class="d-flex align-center ml-auto font-weight-bold" style="font-size: 1.2rem; pointer-events: none">
            <div v-if="!serverStatus" class="ml-2 px-2 py-1" style="color: var(--action-red); background-color: var(--standard-background); border-radius: 4px">
              {{ localization[lang].app.server.indicator.off }}
            </div>
            <div v-else class="ml-2 px-2 py-1" style="color: var(--action-green); background-color: var(--standard-background); border-radius: 4px">
              {{ localization[lang].app.server.indicator.on }}
            </div>
          </div>
          <v-btn :color="$vuetify.theme.themes[appTheme].action_blue" :disabled="!serverStatus" class="ml-2" text @click="reconnect()">
      ...
      <div class="server_messages_container_wrapper" style="position: relative; flex: 1 0 auto; min-height: 18rem; background-color: var(--background-deep)">
        <div
          ...
          "
          class="pa-2"
          style="position: absolute; top: 0; right: 0; bottom: 0; left: 0; overflow-y: auto; border-radius: 0 0 6px 6px; scroll-behavior: smooth"
        >
          <v-row
            v-for="(mes, m) in serverMessages"
            :key="m"
            :style="{
              color: `${$vuetify.theme.themes[appTheme].messageColor[mes[0]] || $vuetify.theme.themes[appTheme].textDefault}`,
            }"
            no-gutters
            >{{ mes[1] }}
          </v-row>
...
export default {
  ...
  methods: {
    ...
    setMessagesScroll() {
      const messagesContainer = document.querySelector('#server_messages_container');

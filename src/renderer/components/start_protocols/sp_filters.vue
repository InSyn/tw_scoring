<template>
  <div
    ...
    style="border-radius: 6px; width: 100%; height: 100%; padding: 16px;user-select: none"
    :style="{
      backgroundColor: $vuetify.theme.themes[appTheme].cardBackgroundRGBA
    }"
  >
    <div style="font-size: 1.2rem; font-weight:bold;padding: .5rem 1rem">
      Настройка фильтров
    </div>
    <div style="display:flex;flex-wrap: wrap;padding: .5rem 1rem">
      ...
      >
        <div
          ...
          @click="
            competition.protocol_settings.start_protocols.result_races.includes(
              race
            )
              ? (competition.protocol_settings.start_protocols.result_races = competition.protocol_settings.start_protocols.result_races.filter(
                  _race => _race.id !== race.id
                ))
              : competition.protocol_settings.start_protocols.result_races.push(
                  race
                )
          "
        >

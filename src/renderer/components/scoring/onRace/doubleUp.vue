<template>
  <v-col class="pa-2" cols="8">
    <div
      class="pa-2"
      style="display: flex; flex-direction: column; height: 100%; width: 100%; border-radius: 6px"
      :style="{
    ...
    >
      <div v-for="(corridor, cor_idx) in corridors" :key="cor_idx" style="flex: 1 0 auto; display: flex; flex-wrap: wrap; align-items: center">
        ...
        <div v-else style="width: 100%; font-size: 1.2rem; font-weight: bold">Waiting next competitor</div>
        <div class="cor_judges" style="display: flex; align-items: center; flex-wrap: wrap; width: 100%">
          <div v-for="judge in corridor" style="display: flex; align-items: center; font-weight: bold; margin-right: 1rem">
            <div>{{ judge.title }}</div>
            <div
              style="margin-left: 0.5rem; padding: 4px 0.5rem; min-width: 3rem; border-radius: 6px"
              :style="{
...
export default {
  ...
  methods: {
    compById(id) {
      const competitor = this.competition.competitorsSheet.competitors.find((_comp) => _comp.id === id);
    ...
    publishResult(competitor_id, cor_idx) {
      ...
      this.competition.result_formula.types[0].doubleUp_competitors[cor_idx] = null;
      if (

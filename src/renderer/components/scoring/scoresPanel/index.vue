...
import ManualMark_dialog from '../dialogs/manualMark_dialog.vue';
import SkiJumpControls from './skiJumpControls.vue';
import AerialsControls from './aerialsControls.vue';
import JudgeTerminalIcon from '../../../assets/icons/judgeTerminal-icon.vue';
import JudgeTerminalControl from './judgeTerminal-control.vue';
import MogulsControls from './mogulsControls.vue';
...
import TimerPanel from '../../timing/timerPanel.vue';
import SxControls from './sx-controls.vue';
...
export default {
  ...
  methods: {
    ...
    publishManualResult({ result }) {
      ...
      const competitorId = this.competition.selected_race.onTrack;
      if (!competitorId) return;

      const competitor = getCompetitorById(this.competition, competitorId);
      ...
      this.competition.selected_race.finished.push(competitorId);
    ...
    publishResult() {
      ...
      if (this.manualResult) {
        this.publishManualResult(this.manualResult);
        return;
      }

      const competitor = this.competition.competitorsSheet.competitors.find((_comp) => _comp.id === competitorId);
      ...
      this.competition.publishResult({
        ...
        status: competitor.race_status,
        ae_code: this.ae_code,
        mg_parameters: { ...this.mgRunData, ...this.mgParameters },
        sjDistance: this.sjDistance,
        sjRamp: this.sjRamp,
        mgTime: this.mgParameters.mgTime,
        mgCode: this.mgParameters.mgCode,
      });
    ...
    <div class="scoringPanel__wrapper">
      ...
      <div class="scoresFrame__wrapper">
        ...
        <sx-controls
          v-if="isQualificationOfDisciplines(competition, ['SX', 'SXT'])"
          :competition="competition"
          @publish-sx-result="publishManualResult"
        ></sx-controls>

        <div
          v-if="!isQualificationOfDisciplines(competition, ['SX', 'SXT'])"
          style="height: 100%; display: flex; flex-direction: column; align-items: flex-end; margin-left: auto; padding: 8px"
        >

  ...
  >
    <v-hover v-slot:default="{ hover }">
      <div
        ...
        "
      >
        <div style="flex: 1 0 auto; display: flex; flex-direction: column; white-space: nowrap; padding: 0.4rem">
          <div
            style="display: flex; align-items: center; font-size: 0.75rem; flex: 0 0 auto; line-height: 1"
            :style="{
          ...
          <div style="flex: 0 0 auto; margin-top: 0.2rem">
            {{ `${event.event_title}. ${competition && competition.mainData.title.stage.value && competition.mainData.title.stage.value.value}` }}
          </div>
        </div>
      </div>
    </v-hover>
    ...
    >
      <div
        ...
        style="padding: 0.4rem 0.8rem; font-weight: bold; cursor: default; user-select: none"
        :style="{
      ...
      >
        {{
          `${competitions.indexOf(competition) + 1} ${competition && competition.mainData.title.stage.value && competition.mainData.title.stage.value.value} ${
            competition && competition.mainData.title.stage.group
          }`
        }}
      </div>
      <v-hover v-for="(_competition, c_id) in competitions" :key="_competition.id" v-slot:default="{ hover }">
        <div
          @click="select_competition($event, _competition)"
          style="padding: 0.4rem 0.8rem; cursor: pointer; user-select: none; transition: border 0.122s, background-color 0.122s"
          :style="[
            {
        ...
        >
          {{
            `${competitions.indexOf(_competition) + 1} ${_competition.mainData.title.stage.value && _competition.mainData.title.stage.value.value} ${

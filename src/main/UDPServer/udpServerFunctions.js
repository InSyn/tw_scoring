...
function createUdpSocket() {
  ...
  udpSocket.on("listening", () => {
    ...
    sendServerMessage({
      color: "green",
      message: `Terminals server started on ${udpSocket.address().address}:${
  ...
  udpSocket.on("message", (message, clientInfo) => {
    ...
    handleTerminalMessage({
      ...
      terminalID:
        client.terminalID !== undefined ? client.terminalID.toString() : null,
    });
  });
...
function checkClientDisconnects({ clients, serverSetup }) {
  ...
  for (const [clientKey, { lastActivity, terminalID }] of clients) {
    if (currentTime - lastActivity > serverSetup.clientTimeout) {
      const inactiveTime_minutes =
        new Date(currentTime - lastActivity) / 1000 / 60;

      sendServerMessage({
        color: "orange",
        message: `Terminal ${terminalID}(${clientKey}) is inactive for ${Math.floor(
          inactiveTime_minutes
        )} minutes`,
      });

      // if (currentTime - lastActivity > serverSetup.clientTimeout * 5) {
      //   clients.delete(clientKey);
      //   sendServerMessage({
      //     color: "red",
      //     message: `Terminal ${terminalID}(${clientKey}) disconnected due to inactivity`,
      //   });
      // }
    }
  }
  ...
  if (serverSetup.isServerRunning) {
    serverSetup.clientsConnectionCheckerId = setTimeout(
      () => checkClientDisconnects({ clients, serverSetup }),
      serverSetup.clientTimeout
    );
  }
}
